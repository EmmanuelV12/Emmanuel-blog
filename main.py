...
class Comment(db.Model):
    ...
    # Create a second Foreign Key, to track the parent post, "blog_posts.id" the blog_posts refers to the tablename of BlogPost.
    post_id = db.Column(db.Integer, db.ForeignKey('blog_posts.id'))
    # Create reference to the BlogPost object, the "comments_post" refers to the comments_post protperty in the BlogPost class.
...
def register():
    ...
    if register_form.validate_on_submit():
        usr_email = register_form.email.data
        is_user_in_db = User.query.filter_by(email=usr_email).first()
        if is_user_in_db:
            flash("You've already signed up with that email, instead login.")
            return redirect(url_for('login'))
        usr_password = register_form.password.data
        salted_and_hashed_password = generate_password_hash(usr_password,
                                                            "pbkdf2:sha256",
                                                            16)
        usr_name = register_form.name.data
        new_user = User(email=usr_email,
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        usr_password = login_form.password.data
